...
export function PendingRequestCard({ connection, onUpdate }: PendingRequestCardProps) {
  ...
  return (
    <Card
      className="group p-3 xs:p-4 hover:shadow-md hover:border-brand-lime/50 transition-all duration-200 active:scale-[0.99] cursor-pointer touch-manipulation overflow-hidden"
    ...
    >
      <div className="flex items-center gap-2 xs:gap-3">
        ...
        <div className="flex-1 min-w-0 overflow-hidden">
          ...
          <p className="text-[10px] xs:text-xs text-gray-400 mt-0.5">{timeAgo}</p>
        </div>

        {/* Actions - Mobile optimized */}
        <div className="flex items-center gap-1 xs:gap-1.5 shrink-0 ml-1" onClick={(e) => e.stopPropagation()}>
          {/* Accept button */}
          <button
            onClick={handleAccept}
            disabled={isLoading !== null}
            className="flex items-center justify-center gap-1 px-2 xs:px-3 py-1.5 xs:py-2 bg-brand-black text-white text-xs font-medium rounded-full hover:bg-brand-black/90 active:scale-95 disabled:opacity-50 transition-all touch-manipulation min-w-[32px] xs:min-w-[40px]"
          >
            {isLoading === 'accept' ? (
          ...
          <button
            ...
            disabled={isLoading !== null}
            className="p-1.5 text-gray-400 hover:text-red-500 hover:bg-red-50 active:bg-red-100 rounded-full disabled:opacity-50 transition-colors touch-manipulation"
          >
            {isLoading === 'reject' ? (
          ...
          <ChevronRight className="w-4 h-4 text-gray-300 hidden sm:block group-hover:text-brand-lime group-hover:translate-x-0.5 transition-all" />
        </div>
      </div>
    </Card>
  );
